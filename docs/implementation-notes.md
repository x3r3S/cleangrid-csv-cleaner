...
3. `getMappingCoverage` also detects an exact source column assigned to more than one CRM field. `cleanDataset` refuses that mapping at the domain boundary, and the UI keeps classification and ready export blocked until it is unique.
4. `cleanDataset` normalizes values, records rule reasons and classifies each row. The UI calls it only when Company and Email are mapped and every mapped source is unique, so an incomplete or semantically conflicting schema cannot masquerade as eight classified records. Contact, Phone and Country are optional: if one is not mapped, its normalized value remains blank and the audit records that exact skip.
...
- Missing Company or Email mappings pause the entire classification and ready export. Reusing any mapped source column also fails closed: the UI identifies both conflicting CRM fields, and audit JSON records `classification: "blocked"` plus `MAPPING_CONFLICT` without `RULES_APPLIED`. Missing Contact, Phone or Country mappings do not block when the remaining mapped sources are unique; the UI and audit name the optional field skipped. When Phone is skipped, no phone validation result is implied and the audit says its normalization and E.164-shape checks did not run.
...
`tests/domain.test.mjs` covers parsing, required/optional and duplicate-source mapping coverage, normalization, phone-shape rules, classification, duplicate detection, summaries, CSV escaping and audit generation. It also proves that a conflicting mapping is refused, skipping Phone avoids phone classification, and restoring Phone brings the review result and full audit wording back. `scripts/verify-evidence.mjs` adds a fixture-level guard: it runs the checked-in awkward CSV through the same domain functions and compares both output queues with the expected files.

The browser suite covers integration seams the domain tests cannot see. It changes Company and Email to **Not mapped**, checks the live `4/5` status, disabled ready export and pending audit download, then restores each mapping. It also assigns E-mail to both Company and Email, checks the accessible conflict, zero classified counts, disabled export and blocked audit JSON, then proves remapping restores `3 / 2 / 3` at wide and mobile sizes. It separately skips and restores Contact, Phone and Country, verifies that export remains enabled, reads the Phone-skip audit JSON and confirms the original counts return. It also reads the downloaded ready CSV, tabs to and activates the native file chooser with Enter and Space, checks the labelled horizontal table region and its mobile hint, proves it can scroll from the keyboard, checks key text sizes and three formerly low-contrast text/background pairs at `≥ 4.5:1`, asserts required/optional accessible names for all five mapping selects, verifies Source and CI links, and checks global page overflow at 1440 × 900 and 390 × 844. These are specific regressions, not an accessibility certification.
